...
class SignupView(View):
    ...
    def post(self, request):
        ...
        # Password Validation check. 1st if > 비밀번호 길이 확인, 2nd if > 비밀번호 복잡도 확인1, 3rd if > 비밀번호 복잡도 확인2
        ...
        if not re.findall('[<`~!@#$%^&*,./?;:>()]+', data['user_pw']):
            return JsonResponse({'message': 'KEY_ERROR Invalid Password(하나 이상의 특수문자를 포함하세요)'}, status = 400)
        
        # Phone number Validation check. 1st if > 전화번호 길이 및 전화번호 형식 확인, 2nd if > 전화번호 중복 확인
